...
test('Business settings saves a synthetic profile and selects it for new quotes', async ({ page }) => {
  ...
  await card.getByLabel('Name').fill('Beispiel Stahl GmbH')
  await card.getByRole('button', { name: 'Save profile' }).click()
  await expect(card).toContainText('Saved revision 1.')
  expect(saved?.definition.schema).toBe('inspr.document-profile.v1')
  await card.getByRole('button', { name: 'Use for new quotes' }).click()
  await expect(card).toContainText('New quotes will use this profile.')
