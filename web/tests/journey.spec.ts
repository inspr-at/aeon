...
test('leaving the journey preserves R1 node details and R2 human decisions',async({page})=>{
  ...
  const request=page.getByRole('article',{name:'Request journey.build',exact:true})
  await request.getByRole('button',{name:'Review request',exact:true}).click()
  await request.getByLabel('Decision reason (optional)',{exact:true}).fill('Reviewed after leaving the journey')
  await request.getByRole('button',{name:'Approve permission',exact:true}).click()
  await expect(page.getByRole('status').filter({hasText:'Decision recorded: approved'})).toBeVisible()
