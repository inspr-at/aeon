...
test('a draft offers no customer capability; issuing sends nothing; the link goes out by copy, QR or PDF', async ({ page }) => {
  ...
  await expect(page.locator('.quote-page').last().getByText('Draft preview')).toBeVisible()
  const draftCode = await page.evaluate(async () => {
    const { decodeQuoteQr } = await import('/tests/quotes/qr-decode-harness.ts')
    return decodeQuoteQr('.quote-page .quote-final-qr')
  })
  expect(draftCode).toBe('DRAFT PREVIEW - NO CUSTOMER LINK')
  ...
  await expect(details.getByRole('textbox', { name: 'Customer link' })).toHaveValue(/\/offers\//)
  const issuedCode = await page.evaluate(async () => {
    const { decodeQuoteQr } = await import('/tests/quotes/qr-decode-harness.ts')
    return decodeQuoteQr('.quote-page .quote-final-qr')
  })
  expect(issuedCode).toBe(await details.getByRole('textbox', { name: 'Customer link' }).inputValue())
