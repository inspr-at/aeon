...
test('global search opens a ticket inside its project', async ({ page }) => {
  ...
  await expect(page.getByRole('combobox', { name: 'Search tickets, projects and actions' })).toBeFocused()
  await page.keyboard.type('oracle')
  await expect(page.getByRole('option').first()).toContainText('Add an Oracle Cloud connector')
  await page.keyboard.press('Enter')
  await expect(page).toHaveURL('/p/PHAROS/PHAROS-12')
