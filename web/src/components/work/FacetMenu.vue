...
const props = defineProps<{ anchor: HTMLElement | null; dimension: Dimension; title: string; options: FacetOption[]; selected: string[] }>()
const emit = defineEmits<{ toggle: [value: string]; clear: []; close: [restoreFocus: boolean] }>()
const term = ref('')
...
<template>
  <FloatingPanel :anchor="anchor" :width="248" :label="`Filter by ${title}`" @close="restore => emit('close', restore)">
    ...
    <FacetOptions :dimension="dimension" :options="shown" :selected="selected" @toggle="value => emit('toggle', value)" />
