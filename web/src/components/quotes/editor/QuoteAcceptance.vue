...
<template>
  <section class="quote-acceptance" aria-label="Acceptance">
    ...
    <QuoteText v-else-if="document.profile?.definition.totals.vat !== 'hidden'" :model-value="document.legal.vat_note ?? ''" label="Umsatzsteuerhinweis" :editable="editable" @update:model-value="editor.editField('legal', { key: 'vat_note', text: $event })" />
...
:global(.quote-document.classic-v1 .quote-acceptance) { font-size: 9.6pt; gap: 4mm; }
:global(.quote-document.classic-v1 .quote-total) { width: 66mm; margin-left: auto; font-size: 11pt; padding-top: 6px; border-color: var(--ink); }
:global(.quote-document.classic-v1 .quote-signatures) { gap: var(--quote-signature-gap); margin-top: var(--quote-signature-lead); }
