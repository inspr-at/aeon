...
<template>
  <div class="graph-controls" role="group" aria-label="Graph controls">
    ...
    <button type="button" class="btn sm graph-motion" :aria-pressed="paused" :aria-label="paused ? 'Resume motion' : 'Pause motion'" @click="emit('pause')"><AppIcon :name="paused ? 'arrow' : 'pause'" :size="14" /><span>{{ paused ? 'Still' : 'Motion' }}</span></button>
    ...
    <slot />
    <details class="graph-menu"><summary class="icon-btn sm flat" aria-label="Graph settings" data-tip="Graph settings"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="M4 7h16M4 17h16" /><circle cx="9" cy="7" r="2.5" fill="var(--surface-raised)" /><circle cx="15" cy="17" r="2.5" fill="var(--surface-raised)" /></svg></summary><div class="graph-menu-panel"><label>Frame rate<select aria-label="Graph frame rate" :value="fps" @change="emit('fps', Number(($event.target as HTMLSelectElement).value) as GraphFPS)"><option :value="60">60 FPS</option><option :value="30">30 FPS</option></select></label></div></details>
