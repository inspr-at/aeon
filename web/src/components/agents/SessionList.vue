...
<template>
  <section class="sessions glass-card" aria-labelledby="sessions-title">
    <header class="card-head">
      ...
      <span v-if="state === 'ready' && loaded" class="sub">{{ live }} live{{ groups.stopped.length ? ` · ${groups.stopped.length} stopped` : '' }}</span>
    </header>

    <div v-if="state === 'unavailable'" class="state">
      <AppIcon name="agent" :size="20" />
      <h3>This server does not report agent sessions yet</h3>
      <p>Approvals and accounts work already. Live sessions appear here once the harness surface is switched on for this workspace.</p>
    </div>
    <div v-else-if="state === 'forbidden'" class="state">
      <AppIcon name="agent" :size="20" />
        ...
        >
          ...
          <span role="cell" class="c-actions">
            <template v-if="view.status.group !== 'stopped'">
              <button
                type="button" class="icon-btn sm flat" :aria-label="`Interrupt ${view.name}`" :data-tip="controlBlock(view, 'interrupt') || 'Interrupt the current turn'"
                :aria-disabled="!!controlBlock(view, 'interrupt')" @click="!controlBlock(view, 'interrupt') && emit('control', view, 'interrupt')"
              ><AppIcon name="pause" :size="14" /></button>
              <button
                type="button" class="icon-btn sm flat stop" :aria-label="`Stop ${view.name}`" :data-tip="controlBlock(view, 'stop') || 'Stop this session'"
                :aria-disabled="!!controlBlock(view, 'stop')" @click="!controlBlock(view, 'stop') && emit('control', view, 'stop')"
              ><AppIcon name="stop" :size="13" /></button>
            </template>
          </span>
        </div>
      </template>
...
.table { display: grid; grid-template-columns: 132px minmax(170px, 1.3fr) minmax(96px, .8fr) minmax(90px, .8fr) minmax(110px, .9fr) 84px 72px 64px; padding: 0 0 8px; }
...
.c-actions { display: inline-flex; justify-content: flex-end; gap: 2px; opacity: .55; }
.row:hover .c-actions, .row.active .c-actions, .row:focus-within .c-actions { opacity: 1; }
.c-actions [aria-disabled="true"] { opacity: .35; cursor: not-allowed; }
...
@container sessions (max-width: 920px) {
  .table { grid-template-columns: 118px minmax(150px, 1.3fr) minmax(96px, .8fr) minmax(100px, .9fr) 80px 64px 64px; }
  .c-account { display: none; }
}
@container sessions (max-width: 760px) {
  .table { grid-template-columns: 112px minmax(140px, 1fr) minmax(90px, auto) 78px 64px; }
...
@container sessions (max-width: 560px) {
  ...
  .row { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; grid-template-areas: "agent agent beat" "state ticket ticket"; row-gap: 6px; min-height: 64px; margin: 0 6px; padding: 10px 10px; }
  .row > span { padding: 0; }
  ...
  .c-account, .c-model, .c-elapsed, .c-actions { display: none; }
}
</style>
