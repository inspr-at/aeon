...
<template>
  <section class="queue glass-card" aria-labelledby="needs-title">
    ...
    <div v-if="loading" class="queue-loading" role="status" aria-label="Loading requests"><span class="skeleton" /><span class="skeleton short" /><span class="skeleton" /></div>
    ...
    <footer v-if="!loading && history.length" class="history">
...
.queue-loading { display: grid; gap: 12px; padding: 12px 18px 24px; }
.queue-loading .skeleton { height: 28px; border-radius: 8px; }
.queue-loading .short { width: 62%; }
