...
export async function refreshPermissions(projectId?: string): Promise<void> {
  ...
  const request = (async () => {
    try {
      ...
      if (started === epoch) cache.set(key, body)
    } catch { if (started === epoch) cache.set(key, null) }
    finally { if (started === epoch) requests.delete(key); revision.value++ }
  })()
  requests.set(key, request)
  return request
}

export function clearPermissions(): void { epoch++; cache.clear(); requests.clear(); revision.value++ }
