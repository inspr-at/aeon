...
export function useTicketList(projectId: Ref<string | null>, filters: Ref<ListFilters>) {
  ...
  async function load() {
    ...
    try {
      const page = await listNodes(apiParams(within, current, { facets: FACETS }))
  ...
  async function fetchMore() {
    ...
    try {
      const page = await listNodes(apiParams(within, filters.value, { facets: FACETS, cursor: cursor.value }))
      if (request !== generation) return
