...
<template>
  <section class="journey-face" aria-label="Project journey workspace">
    <header class="journey-heading"><div><p class="eyebrow">{{ stageLabel(stage) }} · {{ state?.state || 'Loading' }} · {{ owners[stage] }}</p><h1>{{ titles[stage] }}</h1></div><div class="profile-control"><label class="j-label">Profile<select :value="store.journey?.profile" :disabled="blocked || !store.journey" @change="changeProfile"><option v-for="(profile,key) in profiles" :key="key" :value="key">{{ profile.label }}</option></select></label><span v-if="store.journey" class="j-note" :title="profiles[store.journey.profile].line">{{ profiles[store.journey.profile].line }}</span></div><button class="j-icon" aria-label="Refresh journey" :disabled="store.loading || store.busy" @click="store.load()"><JourneyIcon name="refresh" /></button></header>
