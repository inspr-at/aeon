...
const CodexSteerTimeout = 20 * time.Second
...
func DeliverCodexSteer(ctx context.Context, body, target string, stderr io.Writer, clientVersion string) (bool, string, error) {
	...
	if strings.TrimSpace(target) == "" {
		return false, "", errors.New("Codex steer requires a receiver-owned thread target")
	}
	path, err := exec.LookPath("codex")
	if err != nil {
		return false, "", errors.New("Codex delivery requires the codex CLI in PATH")
	}
