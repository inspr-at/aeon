...
func TestPluginDeclaration(t *testing.T) {
	...
	wantPerms := []string{fence.PermNodesContribute, fence.PermStepsApply, fence.PermViewsProvide}
	...
	if len(stored.Manifest.AgentTools) != 0 || len(stored.Manifest.Integrations) != 0 || len(stored.Manifest.BackgroundJobs) != 0 {
		t.Fatal("crm declares no tools, integrations, or jobs")
	}
